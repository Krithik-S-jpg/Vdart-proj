def cricket_scorecard(players):
    ...
    print("'W' for wicket, 'N' for no ball, 'R' for run out")
    print("Enter '-1' to end innings\n")

    while wickets < len(players) - 1:
        event = input(f"Ball {ball_number} (run/W/N/R): ").strip().upper()
        
        # Handle end of innings
        if event == '-1':
            break

        # No ball - 1 extra run, ball not counted
        if event == 'N':
            extras += 1
            print(f"No ball! 1 extra run added. Total extras: {extras}")
            continue  # Don't increase ball_number
        ...
        if event == 'W':  # Wicket
            ...
            # Try to convert event to integer runs
            try:
                runs = int(event)
                if runs < 0 or runs > 6:
                    print("Invalid runs. Enter 0-6, W, N, R, or -1 to end.")
                    continue
            except ValueError:
                print("Invalid input. Enter 0-6, W, N, R, or -1 to end.")
                continue
    ...
    print(f"Total extras (no balls): {extras}\n")
